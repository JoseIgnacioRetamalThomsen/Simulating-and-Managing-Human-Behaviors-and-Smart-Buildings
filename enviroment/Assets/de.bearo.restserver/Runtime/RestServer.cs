...
namespace RestServer
{
    ...
    public class RestServer : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        /// </summary>
        [Tooltip(
            "The port to bind to. Note: Ports <1024 usually require root/administration rights to be able to bind to.")]
        ...
        /// </summary>
        [SerializeField] private ListenAddressUnity _listenAddressUnity;
        ...
        [SerializeField] [Tooltip("Activate debug logging (verbose).")]
        ...
        public virtual void StartServer()
        {
            ...
            _logger.Log(
                $"Starting server on {ListenAddress}:{portStr} with debug enabled: {DebugLog} on GameObject {gameObject.name}");
            if (port < 1024 && port != 0)
            {
                _logger.LogWarning(
                    "Ports <= 1024 usually require administrator or root rights. If binding fails, try a port >1024.",
                    this);
            }
...
#if RESTSERVER_PROFILING && ENABLE_PROFILER
                RestServerProfilerCounters.ThreadingHelperMainThreadBlockTime.Value +=
